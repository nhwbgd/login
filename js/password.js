...
(function($) {
	 window.PasswordUtil = {
			 ...
			 analyzePwd:function(password,obj){
				 ...
				 var target =  obj.find(".intensity");
				 target.find("i").removeClass("bg-red-strength").removeClass("bg-yellow-strength").removeClass("bg-green-strength");

				 if(score < 25 && score >= 0){
					 target.find("i:first").attr("class","bg-red-strength");
				 }else if(score >=25 && score <= 45){
					 target.find("i:lt(2)").attr("class","bg-yellow-strength");
				 }else if(score > 45){
					 target.find("i:lt(3)").attr("class","bg-green-strength");
				 }
				 // target.show();
			 },
